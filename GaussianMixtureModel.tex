...
hard assignm. vs soft. sherical clusters shapes vs covariance matrix. fast vs slow and more iteration. Use K-means as init for EM.\\
K-means is special case of GMM with covariances $\Sigma_j = \sigma^2 * I$. in the limit of $\sigma \rightarrow 0$ recover K-means.

\subsection*{Model Order Selection (AIC / BIC for GMM)}
Trade-off btw data fit (i.e. likelihood $p(\mathbf{X} | \theta)$) and complexity (i.e. \# of free parameters $\kappa(\cdot)$). For choosing $K$:\\
...
\# of free params, fixed covariance matrix: $\kappa(\theta) = K \cdot D + (K - 1)$ ($K$: \# clusters, $D$: $\mathsf{dim}\text{(data)}=\mathsf{dim}(\mu_i)$, $K-1$: \# free clusters, full cov. matrix: $\kappa(\theta) = K(D + \frac{D(D+1)}{2}) + (K - 1)$.\\
Comp. AIC/BIC for different $K$: find smallest. BIC penalizes complexity more.
